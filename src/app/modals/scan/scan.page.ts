...
import { IonContent, ModalController, NavParams, Platform, AlertController, PickerController, LoadingController } from '@ionic/angular';
              ...
              callback: (async () => {
                ...
                setTimeout(() => {
                  if(this.punchCountDialogShow && this.demoNewCardPromo != null && this.demoNewCardPromo != undefined){
                    ...
                    message: 'ePunch Card has been created and added to the campaign. ePunch card progress is ' + this.demoNewCardPromo.punch.punchCount + ' out of ' + this.demoNewCardPromo.punch.punchesRequired + '!',
                    buttons: [{
                      text: 'Okay',
                      handler: (async () => { 
  ...
  */
  ...
  private async processPunchWithEngagement() {
    ...
    if (this.punchFF.getPunchID()) {
      ...
      if (newCardPromo.punch.punchCount >= newCardPromo.punch.punchesRequired) {
        ...
        this.alertController.create({
          header: 'Punch Card Added',
          message: 'ePunch Card has been created and added to the campaign. ePunch card progress is ' + newCardPromo.punch.punchCount + ' out of ' + newCardPromo.punch.punchesRequired + '!',
          buttons: [{
            text: 'Okay',
            handler: (async () => { }).bind(this)
          }]
  ...
  private async processPunchWithoutEngagement() {
    ...
    if (duplicatePunches.length > 0) {
      ...
      punchLoading.dismiss();
      this.alertController.create({
        header: 'No ePunch Card Found',
        message: 'There was no ePunch Card found to this apply this ePunch code to. Do you want to save the ePunch and opt-in later?',
        buttons: [{
          text: 'Yes',
          handler: async () => {
